...
const REGION_CONDITIONS = {
  ...
  "Ventura": { lat: 34.2746, lon: -119.229, station: "9411189", fallbackWater: 62, swell: 3, targets: ["Rockfish", "Whitefish", "Calico Bass", "Yellowtail"] },
...
const api = {
  home: () => fetchJson("/home.json"),
  boats: () => fetchJson("/boat-ratings.json"),
  landings: () => fetchJson("/landings.json"),
  dailyIndex: () => fetchJson("/daily-report-index.json"),
  dailyReport: (file) => fetchJson(`/${file}`),
  biteTrends: () => fetchJson("/bite-trends.json"),
  prices: () => fetchJson("/trip-prices.json"),
  regionConditions: (region, date = todayIso()) => loadRegionConditions(region, date),
...
async function buildRegionForecasts(daysBack = 14) {
...
async function loadRegionConditions(region, date) {
...
async function getNwsWeather(lat, lon, dateString) {
  try {
    ...
    const period = periods.find((p) => String(p.startTime).startsWith(dateString)) || periods[0];
...
async function getOpenMeteoWeather(lat, lon, dateString) {
  try {
    ...
    let index = hourly.time.findIndex((t) => t.startsWith(`${dateString}T06`));
    if (index < 0) index = hourly.time.findIndex((t) => t.startsWith(dateString));
...
function calculateRegionForecast(region, last7Rows, previous7Rows, condition) {
  ...
  const finalScore = Math.round(trendScore * 0.3 + volumeScore * 0.22 + fpaScore * 0.25 + tripScore * 0.1 + moon.score * 0.04 + windScore * 0.04 + tideScore * 0.03 + waterTempScore * 0.02);
...
function calculateConditionsScore(wind, gusts, swell, waterTemp, tideMovement, weather) {
  let score = 78;
  if (wind <= 8) score += 10;
  ...
  if (tideMovement >= 4) score += 8;
  else if (tideMovement < 1.5) score -= 5;
  return Math.max(0, Math.min(100, Math.round(score)));
}
function defaultRegionConditions(region, date) {
  const base = REGION_CONDITIONS[region];
...
function getTopBoats(rows) {
  return aggregateBy(rows, "boat").map(([boat, fish]) => ({ boat, fish })).slice(0, 5);
}
function getTopLandings(rows) {
  return aggregateBy(rows, "landing").map(([landing, fish]) => ({ landing, fish })).slice(0, 5);
}
function aggregateBy(rows, key) {
  const m = /* @__PURE__ */ new Map();
  rows.forEach((r) => {
    const k = r[key];
    if (k) m.set(k, (m.get(k) || 0) + Number(r.total_fish || 0));
  });
  return Array.from(m.entries()).sort((a, b) => b[1] - a[1]);
}
function sumFish(rows) {
  return rows.reduce((t, r) => t + Number(r.total_fish || 0), 0);
}
function sumAnglers(rows) {
  return rows.reduce((t, r) => t + Number(r.anglers || 0), 0);
}
function countTrips(rows) {
  return new Set(rows.map((r) => `${r.trip_date}|${r.landing}|${r.boat}|${r.trip_type}`)).size;
}
...
function getVolumeScore(fish) {
  if (fish >= 1e4) return 92;
  if (fish >= 5e3) return 82;
  if (fish >= 2500) return 70;
  if (fish >= 1e3) return 52;
  if (fish > 0) return 32;
  return 0;
}
...
function getMoonPhaseScore(date = /* @__PURE__ */ new Date()) {
  const knownNewMoon = /* @__PURE__ */ new Date("2000-01-06T18:14:00Z");
  const lunarCycle = 29.53058867;
  const daysSinceNewMoon = (date.getTime() - knownNewMoon.getTime()) / 864e5;
  const age = (daysSinceNewMoon % lunarCycle + lunarCycle) % lunarCycle;
  if (age < 1.84566) return { phase: "New Moon", score: 85, age };
  if (age < 5.53699) return { phase: "Waxing Crescent", score: 65, age };
  if (age < 9.22831) return { phase: "First Quarter", score: 72, age };
  if (age < 12.91963) return { phase: "Waxing Gibbous", score: 60, age };
  if (age < 16.61096) return { phase: "Full Moon", score: 82, age };
  if (age < 20.30228) return { phase: "Waning Gibbous", score: 60, age };
  if (age < 23.99361) return { phase: "Last Quarter", score: 72, age };
  if (age < 27.68493) return { phase: "Waning Crescent", score: 65, age };
  return { phase: "New Moon", score: 85, age };
}
function getWindScore(wind, gusts) {
  if (!wind && !gusts) return 55;
  if (wind <= 6 && gusts <= 12) return 90;
  if (wind <= 10 && gusts <= 18) return 78;
  if (wind <= 15 && gusts <= 24) return 62;
  if (wind <= 20 && gusts <= 30) return 42;
  return 25;
}
...
function getWaterTempScore(temp) {
  if (!temp) return 55;
  if (temp >= 64 && temp <= 68) return 90;
  if (temp >= 61 && temp < 64) return 76;
  if (temp > 68 && temp <= 72) return 76;
  if (temp >= 58 && temp < 61) return 60;
  if (temp > 72 && temp <= 76) return 58;
  return 40;
}
function getTideMovementFeet(tides) {
  const heights = tides.map((t) => Number(t.v)).filter(Number.isFinite);
...
function formatTides(tides) {
  if (!tides.length) return [{ time: "N/A", type: "High Tide", height: "Unavailable" }, { time: "N/A", type: "Low Tide", height: "Unavailable" }];
  return tides.map((tide) => ({ time: new Date(String(tide.t).replace(" ", "T")).toLocaleTimeString("en-US", { hour: "numeric", minute: "2-digit" }), type: tide.type === "H" ? "High Tide" : "Low Tide", height: `${Number(tide.v).toFixed(1)} ft` }));
}
...
function degreesToCompass(degrees) {
  if (degrees === null || degrees === void 0 || Number.isNaN(degrees)) return "W";
  return ["N", "NE", "E", "SE", "S", "SW", "W", "NW"][Math.round(degrees / 45) % 8];
}
function metersToMiles(meters) {
  if (meters === null || meters === void 0 || Number.isNaN(meters)) return null;
  return Number((meters / 1609.344).toFixed(1));
}
function weatherSummary(code) {
  const map = { 0: "Clear", 1: "Mostly clear", 2: "Partly cloudy", 3: "Overcast", 45: "Fog", 48: "Fog", 51: "Light drizzle", 53: "Drizzle", 55: "Heavy drizzle", 61: "Light rain", 63: "Rain", 65: "Heavy rain", 80: "Rain showers", 81: "Rain showers", 82: "Heavy rain showers", 95: "Thunderstorms" };
  return code === void 0 ? "Forecast available" : map[code] || "Forecast available";
}
...
function daysAgo(date, today) {
  ...
  return Math.floor((today.getTime() - date.getTime()) / 864e5);
}
...
function unslugify(s, candidates) {
  return candidates.find((c) => slugify(c) === s);
}
function todayIso() {
