...
async function loadRecentDailyRows() {
  ...
  for (const year of years) {
    try {
      ...
      rows.push(
        ...yearRows.filter(row =>
          recentDates.has(String(row.trip_date || ""))
        )
      );
    } catch (error) {
...
function buildTripTypeDropdown() {
  const select = document.getElementById("tripTypeSelect");
  ...
  select.innerHTML =
    ...
    uniqueTypes
      .map(type => `<option value="${safeAttr(type)}">${safe(type)}</option>`)
      .join("");
}

function buildRegionDropdown() {
  const select = document.getElementById("priceRegionSelect");
  ...
  select.innerHTML =
    ...
    regions
      .map(region => `<option value="${safeAttr(region)}">${safe(region)}</option>`)
      .join("");
}
...
function renderLandingCards() {
  ...
  if (!landingStats.length) {
    grid.innerHTML = `<div class="empty-card">No landing data found.</div>`;
    return;
  }
  ...
  grid.innerHTML = landingStats
    .map(landing => {
      const landingPrices =
        tripPrices.filter(row => row.landing === landing.landing);

      const landingTripTypes =
        tripTypes.filter(row => row.landing === landing.landing);

      const cheapestPrice =
        landingPrices.length
          ? Math.min(
              ...landingPrices.map(x => Number(x.price || 999999))
            )
          : null;
      ...
      return `
        <article class="landing-card-love">
          <div class="landing-card-top">
            ...
            ${
              cheapestPrice && cheapestPrice < 999999
                ? `
            ...
            ${landingPrices
              ...
              .map(price => `
                <div class="landing-price-row">
                  <span>${safe(price.trip_type)}</span>
          ...
          <div class="landing-actions">
            ...
            ${
              landingPrices[0]?.booking_url
                ? `
                  <a
                    href="${safeUrl(landingPrices[0].booking_url)}"
                    target="_blank"
                    rel="noopener"
                  >
                    Book Trip
